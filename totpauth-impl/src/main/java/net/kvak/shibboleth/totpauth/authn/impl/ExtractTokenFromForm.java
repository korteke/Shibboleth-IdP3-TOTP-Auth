...
/**
 * An action that extracts a token code from an HTTP
 * form, creates a {@link TokenUserContext}, and attaches it to the {@link AuthenticationContext}.
 * 
 * @author korteke
 */
...
public class ExtractTokenFromForm extends AbstractExtractionAction {
	...
	/** Class logger. */
	@Nonnull
	private final Logger log = LoggerFactory.getLogger(ExtractTokenFromForm.class.getName());
	...
	public void settokenCodeField(@Nonnull @NotEmpty final String fieldName) {
		log.debug("{} Adding tokenCode to {}", getLogPrefix(), fieldName);
